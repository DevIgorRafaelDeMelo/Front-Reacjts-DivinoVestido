...
export default function AdminPage({ setCurrentPage }) {
  ...
  const [currentPageDiv, setCurrentPageDiv] = useState("agendamento");
  ...
  const handlePageChange = (page) => {
    setCurrentPageDiv(page);
    setMenuOpen(false);
    setShowCalendar(page === "agendamento");
  };
  ...
  const fetchClientes = async () => {
    ...
    const clientesMap = {};
    data.forEach((item) => {
      if (!item.telefone || item.blocked) return;

      if (!clientesMap[item.telefone]) {
        clientesMap[item.telefone] = {
          nome: item.nome,
          telefone: item.telefone,
          agendamentos: [],
        };
      }
      clientesMap[item.telefone].agendamentos.push(item);
    });

    const clientesArray = Object.values(clientesMap).map((c) => ({
      ...c,
      quantidade: c.agendamentos.length,
      maisRecente: c.agendamentos.sort(
        (a, b) => new Date(b.date) - new Date(a.date)
      )[0].date,
    }));

    clientesArray.sort((a, b) => new Date(b.maisRecente) - new Date(a.maisRecente));
          ...
          <div className="flex justify-end items-center space-x-6 relative">
            <div className="relative pt-2">
              ...
              <button
                ...
                className="relative focus:outline-none ms-4"
              >
                <svg
                ...
                <table className="hidden md:table min-w-full bg-white rounded-lg overflow-hidden shadow-md">
                  <thead>
                    <tr className="bg-gradient-to-r from-yellow-500 to-yellow-600 text-white text-sm uppercase tracking-wide">
                      ...
                      <th className="px-6 py-3 text-left">Último Agendamento</th>
                    </tr>
                  </thead>
                  <tbody>
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white rounded-lg shadow-lg w-11/12 md:w-3/4 lg:w-2/3 relative p-6">
                ...
                <table className="min-w-full bg-white rounded-lg overflow-hidden shadow-md">
                  ...
                  <tbody>
                    {clienteSelecionado.agendamentos.map((a) => (
                      <tr
                        key={a.id}
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-sm text-gray-600">{a.evento}</td>
                        <td className="px-6 py-4 text-sm text-gray-600">{a.pessoas}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
